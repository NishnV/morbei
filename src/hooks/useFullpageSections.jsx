...
/**
 * Drives a Home-style "fullpage" slide experience: normal page scroll is
 * locked, and mouse wheel / touch swipe / arrow keys transition between
 * full-viewport sections (position the sections with the returned
 * `sectionStyle(idx)`). If the LAST section (index `totalSections - 1`)
 * renders <Footer showWordmark /> inside a ".fp-footer-section" element,
 * entering/leaving it morphs the navbar logo into the footer wordmark (and
 * back) via WAAPI — render `floatingLogoPortal` once, anywhere in the tree.
 */
export function useFullpageSections(totalSections) {
    ...
    useLayoutEffect(() => {
        ...
        if (activeSection === footerIndex && prev !== footerIndex) {
        ...
        } else if (activeSection !== footerIndex && prev === footerIndex) {
    ...
    const goNext = useCallback(() => setActiveSection(prev => {
        if (isTransitioning.current || prev >= totalSections - 1) return prev;
    ...
    return {
        ...
        handleTouchStart,
        handleTouchEnd,
        floatingLogoPortal: createPortal(floatingLogoEl, document.body),
    };
}
